...
function makeCard(item) {
  let card = `<div class="menu-item my-3">
    <div class="card">
...
function populateCardContainer(filter) {
  var filteredMenu;
  var cards = "";
  ...
  if (filter == null) {
    ...
    filteredMenu = menu.filter((x) => x.category == filter);
  }

  //contruct the items list
  for (var item of filteredMenu) {
    cards += `<div class="col col-lg-3 col-md-4 col-sm-6">
      ${makeCard(item)}
    </div>`;
  }
...
function filterCards(filter) {
  ...
  switch (filter) {
    case "all":
      ...
      break;
    case "starters":
      populateCardContainer("starters");
      break;
    case "mains":
      populateCardContainer("mains");
      break;
    case "desserts":
      populateCardContainer("desserts");
      break;
    case "beverages":
      populateCardContainer("beverages");
      break;
    default:
      break;
  }
...
function filterBySearch() {
  ...
  var searchText = document.getElementById("search-data").value;

  if (searchText == null || searchText == "") {
    filteredMenu = menu;
  } else {
    filteredMenu = menu.filter((x) =>
